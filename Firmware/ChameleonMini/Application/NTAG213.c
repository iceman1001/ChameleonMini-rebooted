...
void NTAG213AppInit(void) {
    ...
    AppCardMemoryRead(&FirstAuthenticatedPage, NTAG213_CONFIG_AREA_START_ADDRESS + CONF_AUTH0_OFFSET, 1);
    AppCardMemoryRead(&Access, NTAG213_CONFIG_AREA_START_ADDRESS + CONF_ACCESS_OFFSET, 1);
...
static uint16_t AppProcess(uint8_t *const Buffer, uint16_t ByteCount) {
    ...
    switch (Cmd) {
        ...
        case CMD_PWD_AUTH: {
            ...
            AppCardMemoryRead(Password, NTAG213_CONFIG_AREA_START_ADDRESS + CONF_PASSWORD_OFFSET, 4);
            ...
            AppCardMemoryRead(Buffer, NTAG213_CONFIG_AREA_START_ADDRESS + CONF_PACK_OFFSET, 2);
